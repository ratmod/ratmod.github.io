...
This is a feature for team modes. Used properly this is a great way to communicate with team mates during game play. Pings also show up in player HUDS to show their direction.

There are two binds to set up:

- `bind \<key\> +ping` - used for location or enemy location. The location ping is a yellow beacon. The enemy ping is a target icon.
- `bind <key> +pingWarn` - used for warning your teammates with an (X) and error sound

Check the HUD section for customizing the ping settings.
...
Ratmod has a zoomscope available for machine gun and rail gun. To enable them use `set cg_drawZoomScope 1`. There are a few settings available to alter the appearance of the scopes:
...
!!! Info "Note"
    These colors apply when `cg_teamModel` or `cg_enemyModel` are set to a bright model (smarine/bright or sarge/bright) or when brightshells are enabled.
...
- `set cg_drawCrosshair 19` - use a number from 1-39
- `set cg_crosshairSize 30` - adjust the size
